...
RUN mkdir -p bin &&\
  LDFLAGS="$LDFLAGS -X github.com/istforks/go-swagger/cmd/swagger/commands.Commit=${commit_hash}" &&\
  LDFLAGS="$LDFLAGS -X github.com/istforks/go-swagger/cmd/swagger/commands.Version=${tag_name}" &&\
